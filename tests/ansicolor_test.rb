...
class ANSIColorTest < Test::Unit::TestCase
  ...
  def test_uncolor
    ...
    assert_equal string, Color.uncolor { string_red }
    assert_equal string, Color.uncolor { string_like_red }
    assert_equal string, ANSITerm::ANSIColor.uncolor { string_red }
    assert_equal string, ANSITerm::ANSIColor.uncolor { string_like_red }
  ...
  def test_attributes
    foo = 'foo'
    for a in ANSITerm::ANSIColor.attributes
      ...
      assert_equal foo, ANSITerm::ANSIColor.uncolor { foo_colored }
      assert_not_equal foo, foo_colored = __send__(a) { foo }
      assert_equal foo, uncolor { foo_colored }
    end
    assert_equal ANSITerm::ANSIColor.attributes, 'foo'.attributes
  end
  ...
  def test_frozen
    string = 'foo'
    red = string.red
    string.extend(ANSITerm::ANSIColor).freeze
